...
public interface RedCustomerMapper {
    @Select("SELECT customer_type, id_number, id_type, name, risk_level, fund_account " +
            "FROM customer WHERE fund_account LIKE CONCAT('%', #{fundAccount}, '%') AND name LIKE CONCAT('%', #{name}, '%')")
